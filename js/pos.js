...
function posDonaAdd(sku, color, rowId) {
  ...
  colorVariants.forEach(v => {
    ...
    if (ex) ex.qty += qtyWanted;
      ...
      costUzs: (typeof getCostUzs === "function" ? getCostUzs(p) : 0), // 2026-07-25: so'mda muzlagan
...
function posQuickAdd(sku, color, packGroup) {
  ...
  if (alreadyInCart) {
    ...
    cart.push({
      ...
      costUzs: (typeof getCostUzs === "function" ? getCostUzs(p) : 0),
...
function confirmVariant() {
  ...
  if (vmSellMode === "karobka") {
    ...
    if (ex) { ex.qty += totalDona; ex.qtyBox = (ex.qtyBox||0) + qtyInput; }
      ...
      costUzs: (typeof getCostUzs === "function" ? getCostUzs(vmProd) : 0),
...
async function checkout() {
  ...
  // \U0001f534 2026-08-15: MUHR SERVERGA YUBORISHDAN OLDIN (\u00a73.5).
  // Avval muhr server chaqiruvidan KEYIN qo'yilardi — server muhrsiz
  // nusxani yozardi, u qaytib kelib lokalni bosardi. Natijada bot ham,
  // sotuv tarixi ham "yagona" ko'rinishga qaytardi (egasining kuzatuvi).
  try {
