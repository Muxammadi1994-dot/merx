...
async function _mlEnsureFull() {
  try {
    ...
    // Avval bu yerda `xarajatlar.select("*")` edi: bo'lim ochilganda butun
    ...
    const _kerakMap = new Map((db.xarajatlar || []).map(x => [String(x.id), x]));
    const _ker = await _faqatKerakli({ jadval: "xarajatlar", sid, kalitUstun: "id",
                                       lokalMap: _kerakMap, belgi: "xarajatlar" });
    const data = _ker.rows;
    try { console.log("💸 Xarajatlar to'ldirish: " + _ker.tortildi + "/" +
...
async function saveEditExp(id) {   // ✅ 2026-08-19: server orqali (async)
  ...
  if (typeof serverSaveRecord === "function") {
    ...
    if (_r && _r.ok === false && _r.code === "conflict") {
      ...
      try {
        console.warn("⚠️ TO'QNASHUV TAHLILI (xarajat)\n" +
