            ...
            <td style="padding:13px 16px">
              <div style="display:flex;gap:4px;justify-content:center;flex-wrap:wrap">
                ...
                <button onclick="saToggleShop('${s.id}')" title="${active?'Bloklash':'Faollashtirish'}"
                  ...
                  border-radius:7px;padding:6px 10px;font-size:12px;cursor:pointer">
                  ${active ? "🔒" : "✅"}</button>
                <!-- 2026-08-03: zaxira tiklash — server allaqachon
                     tayyor edi (list_backups/get_backup/restore_write),
                     lekin klientda oyna YO'Q edi: tiklash imkoni
                     butunlay yopiq turardi. -->
                <button onclick="saOpenBackups('${s.id}')" title="Zaxiralar"
...
// ══════════════════════════════════════════════════════════════
// ZAXIRA TIKLASH (2026-08-03)
// ══════════════════════════════════════════════════════════════
// Server amallari allaqachon bor edi (`list_backups`, `get_backup`,
// `restore_write`), lekin klientda oyna YO'Q edi — ya'ni zaxiradan
// tiklash butunlay imkonsiz edi. Ma'lumot yo'qolganda (do'kon nomi,
// xodim) qo'lda tiklashga to'g'ri kelardi.
//
// ⚠️ TIKLASH XAVFLI AMAL: tanlangan jadvallar zaxiradagi holatga
// QAYTADI. Shuning uchun ikki tasdiq va aniq ogohlantirish bor.

let _saBkShopId = null;
let _saBkList   = [];

// ══════════════════════════════════════════════════════════════
// ZAXIRA → JADVAL MOSLASHTIRUVCHISI (2026-08-03)
// ══════════════════════════════════════════════════════════════
// Zaxira ilovaning ICHKI ko'rinishida saqlanadi (`db.products` —
// JS obyektlari), jadval esa boshqa ustunlarni kutadi.
// To'g'ridan yozib bo'lmaydi: `shop_id` yo'q, maydon nomlari
// boshqacha.
//
// Yechim: majburiy ustunlarni to'ldiramiz, butun yozuvni esa
// `data` ustuniga solamiz. Ilova aynan `data` dan tiklaydi
// (kontekst §12), shuning uchun hech bir maydon yo'qolmaydi.
//
// Majburiy ustunlar (tekshirilgan 2026-08-03):
//   customers, products, staff → id, shop_id, name
//   sales, ombor, debt_payments, xarajatlar → id, shop_id
function _saBkRow(table, rec, shopId) {
  const row = { shop_id: shopId, id: rec.id, data: rec };
  if (table === "products") {
    if (!rec.sku) return null;              // sku noyob kalit — majburiy
    row.sku  = rec.sku;
    row.name = rec.name || "Nomsiz";
    // Rasm `data` ichida YO'Q — o'z ustunlarida (kontekst §5.1)
    if (rec.image)       row.image        = rec.image;
    if (rec.colorImages) row.color_images = rec.colorImages;
  }
  if (table === "customers" || table === "staff") row.name = rec.name || "Nomsiz";
  if (table === "sales") { row.date = rec.date || null; row.remaining = rec.remaining ?? null; }
  return row.id != null ? row : null;
}

async function saOpenBackups(shopId) {
  _saBkShopId = shopId;
  const shop = _saShops.find(x => x.id === shopId);
  const nomi = shop ? (shop.name || shopId) : shopId;

  document.getElementById("sa-bk-ov")?.remove();
  const ov = document.createElement("div");
  ov.id = "sa-bk-ov";
  ov.style.cssText = `position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,.5);
    display:flex;align-items:center;justify-content:center;font-family:'DM Sans',sans-serif`;
  ov.innerHTML = `
    <div style="background:#fff;border-radius:16px;width:620px;max-width:94vw;
      max-height:86vh;display:flex;flex-direction:column;overflow:hidden">
      <div style="padding:18px 22px;background:#0D1B2A;color:#fff;
        display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-weight:700;font-size:15px">💾 Zaxiralar</div>
          <div style="font-size:12px;color:#94A3B8;margin-top:2px">${nomi}</div>
        </div>
        <button onclick="document.getElementById('sa-bk-ov').remove()"
          style="background:none;border:none;color:#fff;font-size:22px;cursor:pointer">×</button>
      </div>
      <div id="sa-bk-body" style="padding:18px 22px;overflow-y:auto;flex:1">
        <div style="text-align:center;color:#94A3B8;padding:30px">Yuklanmoqda...</div>
      </div>
    </div>`;
  ...
  try {
    ...
    _saBkList = (d && d.ok) ? (d.backups || []) : [];
    saRenderBackups();
  } catch (e) {
    const b = document.getElementById("sa-bk-body");
    if (b) b.innerHTML = `<div style="color:#DC2626">Xato: ${e.message}</div>`;
  }
}

function saRenderBackups() {
  const b = document.getElementById("sa-bk-body"); if (!b) return;
  if (!_saBkList.length) {
    b.innerHTML = `<div style="text-align:center;color:#94A3B8;padding:30px">
      Bu do'konda zaxira yo'q</div>`;
    return;
  }
  b.innerHTML = `
    <div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:9px;
      padding:11px 14px;margin-bottom:14px;font-size:12.5px;color:#92400E;line-height:1.5">
      <strong>Diqqat:</strong> tiklash hozirgi ma'lumotni zaxiradagi holatga
      qaytaradi. Zaxiradan keyin kiritilgan yozuvlar yo'qoladi.
      Avval hozirgi holatning zaxirasi borligiga ishonch hosil qiling.
    </div>
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <thead><tr style="background:#F9FAFB">
        <th style="text-align:left;padding:8px 10px;font-size:11px;color:#6B7280">SANA</th>
        <th style="text-align:right;padding:8px 10px;font-size:11px;color:#6B7280">YOZUVLAR</th>
        <th style="width:90px"></th>
      </tr></thead>
      <tbody>
      ${_saBkList.map(bk => `
        <tr style="border-top:1px solid #F3F4F6">
          <td style="padding:9px 10px">${bk.date || "—"}</td>
          <td style="padding:9px 10px;text-align:right;font-weight:600">
            ${(bk.records || 0).toLocaleString("ru-RU")}</td>
          <td style="padding:9px 10px;text-align:right">
            <button onclick="saRestoreBackup('${bk.id}','${bk.date || ""}',${bk.records || 0})"
              style="background:#EFF6FF;border:1px solid #BFDBFE;color:#1D4ED8;
...
async function saRestoreBackup(backupId, date, records) {
  // ⚠️ IKKI TASDIQ — tasodifan bosilmasin
  if (!confirm(`${date} sanasidagi zaxiradan tiklash?\n\n` +
               `${(records||0).toLocaleString("ru-RU")} ta yozuv.\n\n` +
               `Hozirgi ma'lumot SHU HOLATGA qaytadi.`)) return;
  if (!confirm("Oxirgi tasdiq.\n\nZaxiradan keyin kiritilgan yozuvlar " +
               "YO'QOLADI. Davom etamizmi?")) return;

  const b = document.getElementById("sa-bk-body");
  if (b) b.innerHTML = `<div style="text-align:center;padding:30px">
    <div style="font-weight:600">Tiklanmoqda...</div>
    <div id="sa-bk-prog" style="font-size:12.5px;color:#6B7280;margin-top:8px"></div></div>`;
  const prog = (t) => { const e = document.getElementById("sa-bk-prog"); if (e) e.textContent = t; };

  try {
    const d = await _saApi("get_backup", { backupId });
    if (!d || !d.ok || !d.backup) throw new Error(d?.error || "Zaxira o'qilmadi");
    const data = d.backup.data || {};

    // Server ruxsat bergan jadvallar
    const MAP = [
      ["products",      "products"],
      ["customers",     "customers"],
      ["sales",         "sales"],
    ...
    let done = 0, total = 0;
    for (const [tbl, key] of MAP) {
      const src = Array.isArray(data[key]) ? data[key] : [];
      if (!src.length) continue;
      // Ichki ko'rinishdan jadval ustunlariga o'tkazamiz
      const rows = src.map(r => _saBkRow(tbl, r, _saBkShopId)).filter(Boolean);
      if (!rows.length) continue;
      prog(`${tbl}: ${rows.length} ta yozuv...`);
      // Bo'laklab yuboramiz — katta to'plam so'rovni yiqitmasin
      let okAll = true;
      for (let i = 0; i < rows.length; i += 200) {
        const part = rows.slice(i, i + 200);
        const rr = await _saApi("restore_write", {
          shopId: _saBkShopId, table: tbl, rows: part, wipe: false
        });
        if (!rr || !rr.ok) { okAll = false; console.warn("tiklanmadi:", tbl, rr && rr.error); break; }
        prog(`${tbl}: ${Math.min(i + 200, rows.length)}/${rows.length}`);
      }
      const r = { ok: okAll };
      if (r && r.ok) { done++; total += rows.length; }
      else console.warn("tiklanmadi:", tbl, r && r.error);
    }
    prog("");
    if (b) b.innerHTML = `<div style="text-align:center;padding:26px">
      <div style="font-size:15px;font-weight:700;color:#059669">✅ Tiklandi</div>
      <div style="margin-top:8px;color:#374151">${done} ta jadval · ${total.toLocaleString("ru-RU")} yozuv</div>
      <div style="margin-top:14px;font-size:12.5px;color:#6B7280">
        Do'kon ilovasini yangilang — o'zgarish ko'rinadi.</div>
    </div>`;
    showSaToast("✅ Zaxiradan tiklandi");
  } catch (e) {
    if (b) b.innerHTML = `<div style="color:#DC2626;padding:20px">Xato: ${e.message}</div>`;
  }
}
